...
describe("deepQuerySelector()", () => {
	test("Should find and return the first button.", () => {
		...
		const btn = document.querySelector("duplicate-buttons")?.shadowRoot?.querySelector("button")
		const secondButton = document.querySelector("duplicate-buttons")?.shadowRoot?.querySelectorAll("button")[1]

		expect(deepQuerySelector(container, "button")).toBeInstanceOf(HTMLButtonElement)
		expect(deepQuerySelector(container, "button")).toBe(btn)
		expect(deepQuerySelector(container, "button")).not.toBe(secondButton)

		expect(deepQuerySelector(baseElement, "button")).toBeInstanceOf(HTMLButtonElement)
